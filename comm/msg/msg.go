...
func HandleMsg(msg *openwechat.Message, msgChan chan MsgStruct) {
	...
	if msg.IsText() { // 处理文本消息
		...
		//	reply := contextTextBypass(contentText, sender.ID()) //回复的信息
		...
		if contentText == "你长什么样" {
			...
			go chatGPT(msgChan) //掉用ChatGPT接口
		}

	}
}

func chatGPT(msgChan chan MsgStruct) {
	apiKey := global.Conf.ChatGPT.ApiKey
		...
		Run: func(cmd *cobra.Command, args []string) {
			for {
				...
				case msgStruct := <-msgChan:
					questionParam := validateQuestion(msgStruct.msgStr)
					GetResponse(client, ctx, questionParam, msgStruct.msg)
				}
			}
		},
	}
...
func GetResponse(client gpt3.Client, ctx context.Context, quesiton string, msg *openwechat.Message) {
	str := ""
